...
namespace Editor
{
    internal static class AnchorsToCornersMenuItems
    {
        // Hotkey: Alt+A
    ...
    public static class MoveAnchorsToCornersExtensions
    {
        ...
        public static void MoveAnchorsToCorners(this RectTransform rectTransform)
        {
            ...
            Rect anchorRect = new(position.x, position.y, size.x, size.y);
        ...
        {
            Vector2 size = new(ownRectTransform.rect.width, ownRectTransform.rect.height);
            return size;
        }
        ...
        private static Vector2 CalculateAnchorRectPosition(RectTransform ownRectTransform, RectTransform parentRectTransform, float width, float height)
        {
            ...
            Vector2 position = new(newX, newY);
            return position;
        }
    ...
    {
        public static List<T> GetSelectedComponents<T>() where T : Component
        {
            List<T> result = new();
